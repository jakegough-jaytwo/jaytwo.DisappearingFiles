...
DOCKER_TAG?=jaytwo_disappearingfiles
...
build: restore
	dotnet build ./jaytwo.DisappearingFiles.sln
...
unit-test: build
	...
	rm -rf out/coverage
	cd ./test/jaytwo.DisappearingFiles.Tests; \
		dotnet test \
		--results-directory ../../out/testResults \
		--logger "trx;LogFileName=jaytwo.DisappearingFiles.Tests.trx"
	reportgenerator \
...
pack:
	rm -rf out/packed
	cd ./src/jaytwo.DisappearingFiles; \
